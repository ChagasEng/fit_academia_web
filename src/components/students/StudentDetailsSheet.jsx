...
export default function StudentDetailsSheet({ student, onClose }) {
  ...
  return <section className="student-sheet" role="dialog" aria-modal="true" aria-label="Dados do aluno">
    ...
    <dl className="student-details"><div><dt>Tipo</dt><dd><StudentTypeBadge type={student.type} /></dd></div><div><dt>E-mail</dt><dd>{student.email || 'Não informado'}</dd></div><div><dt>WhatsApp</dt><dd>{phone?.numero || 'Não informado'}</dd></div><div><dt>CEP</dt><dd>{address?.cep || 'Não informado'}</dd></div><div className="full"><dt>Endereço</dt><dd>{destination || 'Não informado'}</dd></div><div className="full"><dt>Complemento / referência</dt><dd>{[address?.complemento, address?.referencia].filter(Boolean).join(' · ') || 'Não informado'}</dd></div></dl>
    <div className="route-actions"><button onClick={openHistory}>Ver histórico e pagamentos</button>{whatsappNumber && <button className="whatsapp-button" onClick={() => window.open(`https://wa.me/${whatsappNumber}`, '_blank', 'noopener,noreferrer')}>◉ Abrir WhatsApp</button>}{destination && <><button onClick={() => openRoute('maps')}>Ir com Google Maps</button><button onClick={() => openRoute('waze')}>Ir com Waze</button></>}</div>
  </section>
}
