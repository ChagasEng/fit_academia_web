...
export default function AgendaPage({ token, onLogout }) {
  ...
  return <main className="dashboard-page agenda-page"><header className="dashboard-header"><div className="header-side"><BackButton /><strong>fit<span>academia</span></strong></div><button onClick={onLogout}>Sair</button></header><section className="agenda-content"><div className="agenda-toolbar"><div><p className="eyebrow">AGENDA</p><h1>Minha semana</h1></div><div className="agenda-actions"><button onClick={() => setWeek(addDays(week, -7))}>‹</button><button onClick={() => setWeek(mondayOf(new Date()))}>Hoje</button><button onClick={() => setWeek(addDays(week, 7))}>›</button></div></div><p className="agenda-period">{period}</p><div className="calendar"><div className="calendar-corner" />{days.map((day) => <button key={dateKey(day)} onClick={() => setSelectedDay(day)} className={dateKey(day) === today ? 'calendar-day today' : 'calendar-day'}><span>{weekdays[day.getDay()]}</span><strong>{day.getDate()}</strong></button>)}{hours.map((hour) => <div className="calendar-row" key={hour}><div className="calendar-hour">{String(hour).padStart(2, '0')}:00</div>{days.map((day) => <button type="button" onClick={() => setSelectedDay(day)} className="calendar-cell" key={`${dateKey(day)}-${hour}`}>{appointments.filter((item) => { const start = new Date(item.inicio); return dateKey(start) === dateKey(day) && start.getHours() === hour }).map((item) => <article className="calendar-event" key={item.id}><strong>{item.titulo}</strong><span>{item.student?.nome || 'Atendimento'}</span></article>)}</button>)}</div>)}</div></section>{selectedDay && <section className="day-sheet" role="dialog" aria-modal="true" aria-label="Agendamentos do dia"><div className="day-sheet-header"><div><p className="eyebrow">AGENDA DO DIA</p><h2>{selectedDay.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })}</h2></div><button onClick={() => setSelectedDay(null)} aria-label="Fechar lista">×</button></div><div className="day-appointments">{selectedAppointments.map((item) => <article key={item.id}><time>{new Date(item.inicio).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</time><div><strong>{item.titulo}</strong><span>{item.student?.nome || 'Atendimento'}</span></div></article>)}{selectedAppointments.length === 0 && <p>Nenhum agendamento para este dia.</p>}</div></section>}</main>
}
