...
export default function StudentListPage({ token, onLogout }) {
  ...
  useEffect(() => { const timeout = setTimeout(() => getStudents(token, page, search, type).then((data) => { setStudents(data.students?.data || []); setPagination(data.students || {}); setCounts(data.counts || {}) }).catch(() => setStudents([])), 250); return () => clearTimeout(timeout) }, [token, page, search, type])
